...
def test_add_microstructure_features(sample_data):
    ...
    microstructure_columns = [
        ...
        "OFI_OFI_MA",
        "OFI_OFI_Std",
        "Spread_Bid_Ask_Spread",
        "Spread_Spread_Momentum",
        "Spread_Spread_MA",
        "Spread_Spread_Upper",
        "Spread_Spread_Lower",
        "Spread_Spread_Position",
        "Volume_Price_Trend",
        "Volume_MA_Ratio",
        "Volume_Std",
        "High_Low_Range",
...
def test_feature_engineering_with_intraday_data():
    ...
    # Ensure High >= Low and High >= Close >= Low
    intraday_data["High"] = intraday_data[["Open", "Close"]].max(
        axis=1
    ) + np.random.uniform(0, 2, 50)
    intraday_data["Low"] = intraday_data[["Open", "Close"]].min(
        axis=1
    ) - np.random.uniform(0, 2, 50)
