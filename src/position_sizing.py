...
class PositionSizer:
    ...
    def __init__(self, config_path: str = "config.yml"):
        ...
        self.position_config = self.config.get('transaction_costs', {}).get('position_sizing', {})
    ...
    def calculate_optimal_position_size(
        ...
        **kwargs
        ...
        if strategy == "volatility_targeted":
            ...
            risk_based_size = self.risk_based_sizing(
                portfolio_value, price, volatility
            )
    ...
    def calculate_portfolio_risk_metrics(
        ...
        concentration_ratio = total_position_value / portfolio_value if portfolio_value > 0 else 0
        
        # Calculate weighted portfolio return
        weighted_returns = [pos_size * ret for pos_size, ret in zip(position_sizes, returns)]
        portfolio_return = sum(weighted_returns) / portfolio_value if portfolio_value > 0 else 0
        
        # Calculate portfolio volatility
        portfolio_volatility = np.std(weighted_returns) if len(weighted_returns) > 1 else 0
