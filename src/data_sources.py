...
class AbstractDataSource(ABC):
    ...
    def update_metrics(
        self, response_time: float, success: bool, quality_score: float = 1.0
    ):
...
class CoinGeckoDataSource(AbstractDataSource):
    ...
    def download_data(
        ...
        try:
            ...
            response_time = time.time() - start_time
            self.update_metrics(
                response_time, True, 0.8
            )  # Lower quality due to no OHLC
...
class MultiDataSourceManager:
    ...
    def download_data_with_fallback(
        self, ticker: str, start_date: str, end_date: str, interval: str = "1d"
    ...
    def _fuse_data(self, data_dict: Dict[str, pd.DataFrame]) -> pd.DataFrame:
        ...
        best_source = max(
            data_dict.keys(), key=lambda x: self.sources[x].metrics.data_quality_score
        )
    ...
    def get_best_source(self) -> str:
        ...
        best_source = max(
            self.sources.keys(), key=lambda x: self.sources[x].metrics.success_rate
        )
        return best_source
...
def download_multi_source_data(
    ticker: str,
    start_date: str,
    end_date: str,
    interval: str = "1d",
