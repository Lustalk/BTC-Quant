...
class TransactionCostModel:
    ...
    def calculate_cost_impact_analysis(
        ...
        cost_impact = {
            ...
            'degradation_percentage': (return_degradation / original_total_return) * 100 if original_total_return != 0 else 0,
            'cost_impact_ratio': cost_summary['cost_impact'],
            'fee_impact': cost_summary['total_fees'] / sum([abs(r) for r in original_returns]) if sum([abs(r) for r in original_returns]) > 0 else 0,
            'slippage_impact': cost_summary['total_slippage'] / sum([abs(r) for r in original_returns]) if sum([abs(r) for r in original_returns]) > 0 else 0
        }
