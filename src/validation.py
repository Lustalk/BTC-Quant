...
class WalkForwardValidator:
    ...
    def __init__(
        ...
        """
        self.initial_train_years = (
            initial_train_years or VALIDATION_CONFIG["initial_train_years"]
        )
        self.test_period_months = (
            test_period_months or VALIDATION_CONFIG["test_period_months"]
        )
    ...
    def generate_walk_forward_periods(
        ...
        while current_train_end < data.index[-1]:
            ...
            test_end = min(
                test_start + timedelta(days=test_period_days), data.index[-1]
            )
            test_data = data[
        ...
        if len(periods) > 0:
            logger.info(
                f"Training periods range: {periods[0][0].index[0].date()} to {periods[-1][0].index[-1].date()}"
            )
        else:
            logger.warning("No walk-forward periods generated - insufficient data")
    ...
    def validate_model(
        ...
        for i, (train_data, test_data) in enumerate(periods):
            ...
            try:
                ...
                logger.info(
                    f"Period {i+1}: X_train shape after dropna: {X_train.shape}, X_test shape: {X_test.shape}"
                )

                # Skip if insufficient data
                if len(X_train) < self.min_train_size or len(X_test) == 0:
                    logger.warning(
                        f"Skipping period {i+1}: insufficient data (X_train: {len(X_train)}, min_required: {self.min_train_size})"
                    )
                    continue
        ...
        overall_results = self._calculate_overall_metrics(
    ...
    def _calculate_period_metrics(
        self, y_true: pd.Series, y_pred: np.ndarray, probabilities: np.ndarray
    ) -> Dict:
        """
    ...
    def _calculate_overall_metrics(
        ...
        if len(y_true) == 0 or len(y_pred) == 0 or len(probabilities) == 0:
            return {
                "accuracy": 0.0,
                "precision": 0.0,
                "recall": 0.0,
    ...
    def get_period_summary(self) -> pd.DataFrame:
        ...
        for i, (train_period, test_period) in enumerate(
            zip(self.train_periods, self.test_periods)
        ):
            summary_data.append(
                {
                    "period": i + 1,
    ...
    def plot_validation_periods(self, data: pd.DataFrame):
        ...
        try:
            ...
            for i, (train_period, test_period) in enumerate(
                ...
                # Train period
                train_mask = (data.index >= train_period["start"]) & (
                    data.index <= train_period["end"]
                )
                ax.fill_between(
                    data.index,
                    data["close"].min(),
                ...
                # Test period
                test_mask = (data.index >= test_period["start"]) & (
                    data.index <= test_period["end"]
                )
                ax.fill_between(
                    data.index,
                    data["close"].min(),
