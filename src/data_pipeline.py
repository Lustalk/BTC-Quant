...
class DataPipeline:
    ...
    def download_data(self, force_download: bool = False) -> pd.DataFrame:
        ...
        file_path = os.path.join(
            PATHS_CONFIG["raw_data_dir"], f"{self.symbol}_data.csv"
        )
        ...
        if not force_download and os.path.exists(file_path):
            ...
            # Ensure index is datetime64[ns]
            # Ensure index is datetime64[ns] and handle timezone-aware datetimes
            self.data.index = pd.to_datetime(self.data.index, utc=True).tz_localize(None)
            return self.data

        logger.info(
            f"Downloading {self.symbol} data from {self.start_date} to {self.end_date}"
        )
        ...
        try:
            ...
            if self.data.empty:
                logger.warning(f"No data downloaded for {self.symbol}. Creating sample data for testing.")
    ...
    def preprocess_data(self, force_download: bool = False) -> pd.DataFrame:
        ...
        essential_columns = ['returns', 'target', 'volatility_20d']
        self.data = self.data.dropna(subset=essential_columns)
        final_rows = len(self.data)

        logger.info(
            f"Preprocessing complete: {initial_rows - final_rows} rows removed due to NaN values"
        )
        if final_rows > 0:
            logger.info(
                f"Final dataset: {final_rows} observations from {self.data.index[0].date()} to {self.data.index[-1].date()}"
            )
        else:
    ...
    def get_train_test_split(
        ...
        logger.info(
            f"Train set: {len(train_data)} observations until {train_end_date.date()}"
        )
        logger.info(
            f"Test set: {len(test_data)} observations from {(train_end_date + timedelta(days=1)).date()}"
        )
