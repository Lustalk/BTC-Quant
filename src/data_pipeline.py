...
def download_multi_timeframe_data(ticker: str, start_date: str, end_date: str, 
...
def _download_yfinance_data(ticker: str, start_date: str, end_date: str, 
    ...
    if yf_interval not in valid_intervals:
        raise ValueError(f"Invalid interval for yfinance: {yf_interval}. Valid intervals: {valid_intervals}")
    
    # Download data with extended range to ensure we get enough data
    extended_start = _extend_start_date(start_date, interval)
    
    try:
        data = ticker_obj.history(start=extended_start, end=end_date, interval=yf_interval)
        ...
        if interval in ["1h", "4h"]:
            ...
            data = ticker_obj.history(start=retry_start, end=end_date, interval=yf_interval)
            
            if data.empty:
                raise ValueError(f"No data found for ticker {ticker} even with shorter period")
...
def ensure_data_completeness(data_dict: Dict[str, pd.DataFrame], 
                           min_years: int = 1) -> Dict[str, pd.DataFrame]:
    """
    ...
    for timeframe, data in data_dict.items():
        ...
        if len(data) < min_data_points:
            print(f"⚠️  Warning: {timeframe} data only has {len(data)} points, need at least {min_data_points}")
            # For intraday data, we're more lenient due to Yahoo Finance limitations
            if timeframe in ["1h", "4h"] and len(data) >= min_data_points * 0.5:  # At least 50% of required
                print(f"✅ Accepting {timeframe} data with {len(data)} points (Yahoo Finance limitation)")
        ...
        if quality['quality_score'] < 0.95:
            print(f"⚠️  Warning: {timeframe} data quality score is {quality['quality_score']:.3f}")
