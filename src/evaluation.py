...
class PerformanceEvaluator:
    ...
    def calculate_strategy_returns(self, 
        ...
        forward_returns = data['close'].shift(-DATA_CONFIG['target_horizon']) / data['close'] - 1
    ...
    def backtest_strategy(self, 
        ...
        # Calculate returns
        self.strategy_returns = self.calculate_strategy_returns(data, predictions, probabilities)
        ...
        information_ratio = (strategy_aligned.mean() - benchmark_aligned.mean()) / tracking_error * np.sqrt(252)
    ...
    def save_results(self, filepath: str = None) -> str:
        ...
        """
        if filepath is None:
            filepath = os.path.join(PATHS_CONFIG['results_dir'], 'model_performance.json')
    ...
    def print_summary(self):
        ...
        metrics = [
            ('Total Return (%)', strategy_metrics['total_return'] * 100, 
             benchmark_metrics['total_return'] * 100),
            ('Annualized Return (%)', strategy_metrics['annualized_return'] * 100, 
             benchmark_metrics['annualized_return'] * 100),
            ...
            ('Max Drawdown (%)', strategy_metrics['max_drawdown'] * 100, 
             benchmark_metrics['max_drawdown'] * 100),
            ('Win Rate (%)', strategy_metrics['win_rate'] * 100, 
             benchmark_metrics['win_rate'] * 100)
        ]
        
        for metric, strategy_val, benchmark_val in metrics:
            alpha = strategy_val - benchmark_val
            print(f"{metric:<25} {strategy_val:<15.2f} {benchmark_val:<15.2f} {alpha:<10.2f}")
