...
def run_enhanced_analysis():
    ...
    try:
        ...
        grid_params, grid_results = hyperopt.grid_search_validation(
            X, y, data_ready.index
        )
        ...
        feature_importance = feature_selector.calculate_feature_importance(
            X, y, data_ready.index
        )

        # Perform RFE
        rfe_results = feature_selector.recursive_feature_elimination(
            X, y, data_ready.index
        )

        # Get optimal feature set
        optimal_features, combined_df = feature_selector.get_optimal_feature_set(
            X, y, data_ready.index
        )
        ...
        enhanced_validator = EnhancedWalkForwardValidator(
            initial_train_years=2, test_period_months=3, min_train_size=500
        )
        ...
        validation_results = enhanced_validator.validate_model(
            data=data_ready, model=model, feature_columns=optimal_features
        )

        logger.info(
            f"Enhanced validation completed: {len(validation_results['period_metrics'])} periods"
        )
        ...
        for period_metrics in validation_results["period_metrics"]:
            ...
            optimal_threshold = (
                threshold_optimizer.calculate_optimal_threshold_for_returns(
                    returns=period_data["returns"],
                    predictions=validation_results["daily_predictions"][
                        validation_results["daily_predictions"]["period"]
        ...
        cumulative_returns = pd.DataFrame(
            {
                "date": data_ready.index,
                "strategy_cumulative": (
                    1 + backtest_results["strategy_returns"]
                ).cumprod(),
                "buy_hold_cumulative": (1 + data_ready["returns"]).cumprod(),
                "excess_cumulative": (
                    1 + backtest_results["strategy_returns"]
                ).cumprod()
                / (1 + data_ready["returns"]).cumprod(),
            }
        )
        cumulative_returns.to_csv("exports/cumulative_returns.csv", index=False)

        # Step 13: Comprehensive Strategy Analysis
        logger.info("Step 13: Comprehensive strategy analysis...")
        
        # Run comprehensive analysis addressing the three key questions
        ...
        logger.info("Step 14: Analysis summary...")
        ...
        logger.info(
            f"Average Strategy Return: {overall_metrics['strategy_return']:.3f}"
        )
        ...
        if simulation_results:
            ...
            logger.info(
                f"Positive Excess Returns: {(sim_df['excess_return'] > 0).mean():.1%}"
            )
...
def run_quick_enhanced_test():
    ...
    optimal_features, _ = feature_selector.get_optimal_feature_set(
        X, y, data_ready.index, max_features=10
    )

    # Quick validation
    enhanced_validator = EnhancedWalkForwardValidator(
        initial_train_years=1, test_period_months=6, min_train_size=200
    )
    ...
    simulation_results = monte_carlo.simulate_strategy_performance(
        data_ready["returns"], validation_results["daily_predictions"]["probability"]
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Enterprise-Grade BTC Trading Strategy Analysis",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python main.py                           # Run full analysis
  python main.py --test                    # Quick test with limited data
  python main.py --start-date 2020-01-01  # Custom start date
  python main.py --model-type xgboost      # Specify model type
  python main.py --verbose                 # Enable debug logging
    ...
    parser.add_argument(
        "--test",
        action="store_true",
        help="Run quick test with limited data (faster execution)",
    )
    ...
    parser.add_argument(
        "--simulations", type=int, default=500, help="Number of Monte Carlo simulations"
    )
    parser.add_argument(
        "--output-dir", type=str, default="exports", help="Output directory for results"
    )
